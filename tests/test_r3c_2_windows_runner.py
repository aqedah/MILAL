...
class R3c2WindowsRunnerTests(unittest.TestCase):
    def exercise(self, shell, default=False, self_status=0, presentation_status=0,
        ...
        with tempfile.TemporaryDirectory(prefix="milal r3c2 runner ") as temp:
            root = Path(temp)
            for name in ("scripts", "src", "caller"):
                (root / name).mkdir()
            runner = root / "scripts/run_milal_r3c_2_windows.ps1"
            shutil.copyfile(ROOT / "scripts" / runner.name, runner)
            interpreter = Path(sys.executable)
            if default:
                venv.EnvBuilder(with_pip=False).create(root / ".venv")
                interpreter = root / ".venv/Scripts/python.exe"
            for name in ("milal_r3c_0_2_reviewability.py", "milal_r3c_1_review_units.py"):
                (root / "src" / name).touch()
...
'''
            ...
            (root / "src/milal_r3c_2_compact_review.py").write_text(script)
            ...
            command = [shell, "-NoProfile", "-NonInteractive", "-ExecutionPolicy", "Bypass", "-File", str(runner),
                       "-R3c1Zip", str(source), "-OutputDir", str(output)]
            if not default:
            ...
            if self_status:
                ...
                return
            self.assertEqual(calls[1]["args"], ["--r3c1-zip", str(source), "--output-dir", str(output)])
