# Running Q1.4 composite source binding

Read [the contract](MFR_0_2R_Q14_SPEC.md) and HANDOFF first. Q1.4 consumes exact
...
$runner = Join-Path $repo 'src\milal_q14_runner.py'
...
$q13 = 'D:\MILAL_runs\mfr02r_q13_20260926\release_a'
$work = 'D:\MILAL_runs\mfr02r_q14_reproduce'
$a = Join-Path $work 'release_a'
$b = 'F:\MILAL_runs\mfr02r_q14_reproduce\release_b'
...
$env:TMP = $env:TEMP
& $py -B -X utf8 -m unittest discover -s tests -p test_milal_q14.py
...
foreach ($destination in @($a, $b)) {
    & $py -B -X utf8 $runner --source $source --q11 $q11 --q12 $q12 --q13 $q13 --out $destination --synthetic (Join-Path $synthetic 'receipt.json') --regression $regression
    if ($LASTEXITCODE -ne 0) { throw "Q1.4 execution failed: $destination" }
}
& $py -B -X utf8 $runner --release $a $b
if ($LASTEXITCODE -ne 0) { throw 'Release verification failed' }
Get-Content (Join-Path $a '22_q14_gates.csv')
Write-Host "Report: $a\20_q14_method_report.md"
...
is not human acceptance. Q1.5 is not executed automatically.
