...
if (-not $Out) { $Out = Join-Path $repo ('results\hsa3_ana_0_1_' + (Get-Date -Format 'yyyyMMdd_HHmmss_fff')) }
$runArgs = @('-B', '-X', 'utf8', (Join-Path $repo 'src\milal_hsa3_ana_response_frame.py'), '--out', $Out)
...
Write-Host 'HSA3-ANA.0.1 surface construction audit; no human parentage decisions'
& $Python @runArgs
$runExit = $LASTEXITCODE
if ($runExit -ne 0) { Write-Error "HSA3-ANA.0.1 stopped: $runExit" -ErrorAction Continue }
else {
