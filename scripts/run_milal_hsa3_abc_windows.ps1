...
if (-not $Out) { $Out = Join-Path $repo ('results\hsa3_abc_' + (Get-Date -Format 'yyyyMMdd_HHmmss_fff')) }
$runArgs = @('-B', '-X', 'utf8', (Join-Path $repo 'src\milal_hsa3_abc_seam_adjudication.py'), '--out', $Out)
if ($SelfTest) { $runArgs += '--self-test' }
if ($Archive) { $runArgs += @('--archive', $Archive) }
Write-Host 'HSA3-A/C composition adjudication; F/G remain unreviewed'
& $Python @runArgs
$runExit = $LASTEXITCODE
if ($runExit -ne 0) { Write-Error "HSA3-A/C stopped: $runExit" -ErrorAction Continue }
else {
