...
if (-not $Out) { $Out = Join-Path $repoRoot ('results\mfr01b_windows_' + (Get-Date -Format 'yyyyMMdd_HHmmss')) }
$pipeline = Join-Path $repoRoot 'src\milal_mfr01b_pipeline.py'
& $Python -B -X utf8 -m unittest discover -s (Join-Path $repoRoot 'tests') -p test_mfr_0_1b.py
if ($LASTEXITCODE -ne 0) { throw 'MFR.0.1b stage tests failed' }
...
if ($LASTEXITCODE -ne 0) { throw 'Synthetic execution failed' }
...
if ($receipt.tests_run -lt 2026 -or $receipt.failures -ne 0 -or $receipt.errors -ne 0 -or $receipt.skipped -ne 0) { throw 'Regression receipt is not PASS' }
