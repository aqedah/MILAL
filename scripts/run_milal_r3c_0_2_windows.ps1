...
$scriptPath = Join-Path $repoDir 'src/milal_r3c_0_2_reviewability.py'
if (-not $PilotConfig) { $PilotConfig = Join-Path $repoDir 'config/r3c_0_2_job_pilot.json' }
...
try {
    ...
    $logStream.Dispose()
    $logReady = $true
    Write-RunLog "MILAL R3c.0.2 Windows; started=$([DateTime]::UtcNow.ToString('o')); seed=$Seed"
    foreach ($path in @($PythonExe, $scriptPath, $PilotConfig, $R3b2Zip, $R3b3Zip,
