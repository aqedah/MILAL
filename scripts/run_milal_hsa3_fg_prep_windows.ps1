...
if (-not $Out) { $Out = Join-Path $repo ('results\hsa3_fg_prep_' + (Get-Date -Format 'yyyyMMdd_HHmmss_fff')) }
$runArgs = @('-B', '-X', 'utf8', (Join-Path $repo 'src\milal_hsa3_fg_prep.py'), '--out', $Out)
...
Write-Host 'HSA3-FG-PREP surface construction audit; no human parentage decisions'
& $Python @runArgs
$runExit = $LASTEXITCODE
if ($runExit -ne 0) { Write-Error "HSA3-FG-PREP stopped: $runExit" -ErrorAction Continue }
else {
