...
if (-not $Out) { $Out = Join-Path $repo ('results\hsa3_de_' + (Get-Date -Format 'yyyyMMdd_HHmmss_fff')) }
$runArgs = @('-B', '-X', 'utf8', (Join-Path $repo 'src\milal_hsa3_de_seam_adjudication.py'), '--out', $Out)
if ($SelfTest) { $runArgs += '--self-test' }
if ($Archive) { $runArgs += @('--archive', $Archive) }
Write-Host 'HSA3-D/E researcher seam adjudication; Q3 remains deferred'
& $Python @runArgs
$runExit = $LASTEXITCODE
if ($runExit -ne 0) { Write-Error "HSA3-D/E stopped: $runExit" -ErrorAction Continue }
else {
