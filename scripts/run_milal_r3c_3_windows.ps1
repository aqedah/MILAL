# R3c.3 reads a frozen result ZIP. No BHSA installation or configuration is needed.
[CmdletBinding()]
param(
    [Parameter(Mandatory = $true)][string]$R3c1Zip,
    [Parameter(Mandatory = $true)][string]$R3c2Zip,
...
$scriptPath = Join-Path $repoDir 'src/milal_r3c_3_signature_context.py'
...
try {
    ...
    $R3c2Zip = Get-FullPath $R3c2Zip
    $R3b2Zip = Get-FullPath $R3b2Zip
    $R3b3Zip = Get-FullPath $R3b3Zip
    ...
    $logStream.Dispose()
    $logReady = $true
    Write-RunLog "MILAL R3c.3 Windows presentation; started=$([DateTime]::UtcNow.ToString('o'))"
    foreach ($path in @($PythonExe, $scriptPath, $R3c1Zip, $R3c2Zip, $R3b2Zip, $R3b3Zip,
                        (Join-Path $repoDir 'src/milal_r3c_2_compact_review.py'),
    ...
    try {
        ...
        Invoke-Python @($scriptPath, '--r3c1-zip', $R3c1Zip, '--r3c2-zip', $R3c2Zip, '--r3b2-zip', $R3b2Zip, '--r3b3-zip', $R3b3Zip, '--output-dir', $OutputDir)
